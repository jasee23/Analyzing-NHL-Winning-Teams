...
columns_to_keep_dict = {
    ...
    "Misc": [34, 35, 36, 37, 38, 39, 82, 83, 84, 85, 86, 87],
    "Radar": [7, 8, 9]               # xGoalsPercentage, corsiPercentage, fenwickPercentage
}

columns_to_keep_list = [0, 5, 7, 8, 9, 12, 21, 22, 23, 24, 25, 40, 41, 42, 46, 47, 48, 69, 70, 71, 72, 73, 80, 88, 89, 90, 94, 95, 96, 34, 35, 36, 37, 38, 39, 82, 83, 84, 85, 86, 87]
...
def plot_misc_heatmap(df, title):
    plt.figure(figsize = (10, 8))
    sns.heatmap(df[["penaltiesFor", "faceOffsWonFor", "hitsFor", "takeawaysFor", "giveawaysFor", "penaltiesAgainst", "faceOffsWonAgainst", "hitsAgainst"]].corr(), annot = True, cmap = "coolwarm", fmt = ".2f")
    plt.title(title)
    plt.show()
